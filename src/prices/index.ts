...
export function getUsdPricePerToken(tokenAddr: Address): CustomPriceType {
  ...
  if (tokenAddr.equals(Address.fromString(RETH_ADDRESS))) {
    const rethPriceContract = rocketTokenRETH.bind(Address.fromString(RETH_ADDRESS));
    ...
    const tokenPrice: BigDecimal = (ethPrice.usdPrice.times(exchangeRateDiv)).div(BigDecimal.fromString('1000000'))
